...
RequestのなかにURLクエリーつまり `?name=値` の形で埋め込まれた値を取り出しています。まずURL文字列を link:https://deno.land/api@v1.39.1?s=URL[`URL`]オブジェクトに変換する。URLクエリーつまり `name=decoy` がURLオブジェクトの `searchParams` プロパティに変換されているので、そのなかから `name` プロパティの値を取り出しています。
